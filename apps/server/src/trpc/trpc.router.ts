    ...
    exportMarkdown: this.trpcService.protectedProcedure
      ...
      .mutation(async ({ input: id }) => {
        try {
          ...
          const { obsidianPath } = this.configService.get<ConfigurationType['feed']>(
            'feed',
          )!;
          ...
          try {
            if (obsidianPath) {
              ...
              this.logger.log(`Auto-saved to Obsidian during export: ${filePath}`);
            }
          } catch (err: any) {
            this.logger.error(`Auto-save to Obsidian failed during export: ${err.message}`);
          }
